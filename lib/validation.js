...
export const transacaoSchema = z.object({
  ...
  valor: z.number().min(-99999).max(999999),
...
export const produtoEmprestimoSchema = z.object({
  nome: z.string().min(1, 'O nome do produto é obrigatório').max(150, 'Nome muito longo')
})

export const emprestimoSchema = z.object({
  nome: z.string().min(1, 'Nome do cliente é obrigatório').max(200),
  item_nome: z.string().min(1, 'Selecione um item'),
  quantidade: z.number().int().positive('A quantidade deve ser maior que zero'),
  data_emprestimo: z.string().min(1),
  hora: z.string().min(1),
  data_iso: z.string().datetime({ message: 'Data ISO inválida' }),
  devolvido: z.boolean().optional()
})

export const emprestimoUpdateSchema = z.object({
  devolvido: z.boolean({ required_error: 'O status de devolução é obrigatório' })
})