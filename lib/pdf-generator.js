...
export function generateClientePDF(cliente, transacoes, dataInicio) {
  const doc = new jsPDF()
  ...
  doc.text(
    `Período: ${new Date(dataInicio).toLocaleDateString('pt-BR')} até ${dataFim.toLocaleDateString('pt-BR')}`,
    20,
    61
  )
