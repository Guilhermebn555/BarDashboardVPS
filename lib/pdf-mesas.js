...
const handlePrintPDF = (mesa, total) => {
    ...
    const colorPrimary = [41, 37, 36] 
    const colorSecondary = [245, 245, 245] 
    const colorAccent = [22, 163, 74] 
    ...
    const tableBody = mesa.itens.map(item => {
      const precoUnitarioExibido = item.isKg 
        ? (item.preco / item.quantidade) 
        : item.preco;

      const subtotalLinha = item.isKg 
        ? item.preco 
        : (item.preco * item.quantidade);

      const qtdFormatada = item.isKg 
        ? `${parseFloat(item.quantidade).toFixed(3).replace('.', ',')} kg` 
        : item.quantidade;

      return [
        item.nome,
        formatCurrency(precoUnitarioExibido),
        qtdFormatada,
        formatCurrency(subtotalLinha)
      ]
    })
    ...
    autoTable(doc, {
      ...
      didParseCell: function (data) {
        if (data.section === 'body' && data.column.index === 3) {
           const rawVal = data.cell.raw.toString().replace('R$', '').trim().replace('.', '').replace(',', '.')
